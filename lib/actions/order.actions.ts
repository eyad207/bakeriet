...
Vi har gleden av å informere deg om at bestillingen din med ID ${
          order._id
        } har blitt ${order.isDelivered ? 'levert' : 'markert som ikke levert'}.

Takk for at du handlet hos oss!

Med vennlig hilsen,
Bakeriet Kundeservice`,
      })
    }
    revalidatePath(`/account/orders/${orderId}`)
    return {
...
Vi har gleden av å informere deg om at bestillingen din med ID ${
          order._id
        } har blitt ${order.isShipped ? 'sendt' : 'markert som ikke sendt'}.
